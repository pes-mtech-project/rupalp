...
for variant in $VARIANTS; do
  config_path="config/tsla_gpt35_top5_${variant}_config.toml"
  train_checkpoint="data/06_train_checkpoint/tsla_rq1_gpt35_${variant}"
  train_result="data/05_train_model_output/tsla_rq1_gpt35_${variant}"
  test_checkpoint="data/06_test_checkpoint/tsla_rq1_gpt35_${variant}"
  test_result="data/05_test_model_output/tsla_rq1_gpt35_${variant}"

  if [[ ! -f "$config_path" ]]; then
    echo "Missing config: $config_path"
    echo "Run: python3 prepare_tsla_rq1_risk_variants_gpt35.py"
    exit 1
  fi

  mkdir -p "$train_checkpoint" "$train_result" "$test_checkpoint" "$test_result"

  echo "== TSLA RQ1 GPT-3.5 ${variant}: train =="
  python3 run.py sim \
  ...
  echo "== TSLA RQ1 GPT-3.5 ${variant}: test =="
  python3 run.py sim \
    --market-data-path "$MARKET_DATA_PATH" \
