...
for ticker in $TICKERS; do
  ...
  config_path="config/${lower}_gpt_rq1_self_adaptive_new_config.toml"
  market_data="data/06_input/subset_symbols_${ticker}.pkl"
  train_checkpoint="data/06_train_checkpoint/${lower}_rq1_gpt35_self_adaptive_new"
  train_result="data/05_train_model_output/${lower}_rq1_gpt35_self_adaptive_new"
  test_checkpoint="data/06_test_checkpoint/${lower}_rq1_gpt35_self_adaptive_new"
  test_result="data/05_test_model_output/${lower}_rq1_gpt35_self_adaptive_new"

  if [[ ! -f "$config_path" ]]; then
    echo "Missing config: $config_path"
    echo "Run: python3 prepare_rq1_gpt35_self_adaptive_new_configs.py"
    exit 1
  fi

  mkdir -p "$train_checkpoint" "$train_result" "$test_checkpoint" "$test_result"

  echo "== ${ticker} RQ1 GPT-3.5 self-adaptive(new): train =="
  python3 run.py sim \
  ...
  echo "== ${ticker} RQ1 GPT-3.5 self-adaptive(new): test =="
  python3 run.py sim \
    --market-data-path "$market_data" \
